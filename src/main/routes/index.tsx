...
function TodayRoute() {
  ...
  const country = settingsQuery.data?.country ?? healthQuery.data?.country ?? "US";
  ...
  const changesQuery = useQuery({
    ...
    queryFn: () =>
      messagingClient.sendMessage<{
        changes: {
          addedItems: Array<{ id: string; title: string; acquisitionDate?: string }>;
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 overflow-y-auto px-2 pb-10 pt-2">
      <section className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm text-muted-foreground">Epic Games Store assistant</p>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <HealthCard
          ...
          detail={health?.lastSyncError ?? "Cookie auth from store.epicgames.com"}
        />
        <HealthCard
          icon={<CheckCircle2 />}
          label="Owned items"
          value={healthQuery.isLoading ? "..." : String(health?.ownedItemCount ?? 0)}
        ...
        <HealthCard
          ...
          detail={health?.overlayEnabled ? "Store overlay active" : "Overlay disabled"}
        />
      </div>
        ...
        <Card>
          ...
          <CardContent>
            {freeGamesQuery.isLoading ? (
              ...
              <EmptyState title="Free games unavailable" detail="egdata did not return free-game data right now." />
            ) : freeGames.length === 0 ? (
              <EmptyState title="No free games found" detail="Check back after the next Epic rotation." />
            ) : (
          ...
          <CardContent className="space-y-4">
            {changesQuery.isLoading ? (
              ...
              <>
                <MetricLine
                  label="New items"
                  value={changesQuery.data?.changes.addedItems.length ?? 0}
                />
                <MetricLine
                  label="Removed IDs"
                  value={changesQuery.data?.changes.removedItemIds.length ?? 0}
                />
                <MetricLine
                  label="Updated metadata"
                  value={changesQuery.data?.changes.updatedItemIds.length ?? 0}
                />
                <div className="space-y-2">
                  {(changesQuery.data?.changes.addedItems ?? [])
                    .slice(0, 4)
                    .map((item) => (
                      <div
                        key={item.id}
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        {hasChildren ? children : <EmptyState title={emptyTitle} detail={emptyDetail} />}
      </CardContent>
    </Card>
  );
}
    ...
    <div className="overflow-hidden rounded-lg border border-border bg-secondary/20">
      ...
      <div className="space-y-3 p-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {offer.developerDisplayName ?? offer.publisherDisplayName ?? "Unknown"}
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={onWatch} disabled={isWatched}>
            <Star />
            {isWatched ? "Watched" : "Watch"}
          </Button>
          <Button size="sm" variant="ghost" asChild>
            <a href={getEgdataOfferUrl(offer.id)} target="_blank" rel="noreferrer">
              <ExternalLink />
            </a>
          </Button>
        </div>
...
function OfferRow({
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border/70 p-2">
      {image && (
        <img src={image.url} alt={offer.title} className="h-14 w-11 rounded object-cover" />
      )}
      ...
      <Button size="icon" variant="ghost" onClick={onWatch} disabled={isWatched}>
...
function WatchlistRow({ item }: { item: WatchlistItem }) {
  return (
    <div className="flex items-center gap-3 rounded-lg border border-border/70 p-2">
      {item.imageUrl && (
        <img src={item.imageUrl} alt={item.title} className="h-14 w-11 rounded object-cover" />
      )}
      <div className="min-w-0 flex-1">
