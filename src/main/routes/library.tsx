...
function LibraryRoute() {
  ...
  const filterOptionsQuery = useQuery({
    ...
    queryFn: () => messagingClient.getLibraryFilterOptions(),
  });
...
function LibraryGridCard({
  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent>
        <ContextMenuItem onClick={() => copyText(item.id)}>
          Copy ID
        </ContextMenuItem>
          ...
          {items.map((item) => (
            <tr key={item.id} className="border-b border-border/60">
              ...
              <td className="py-3 pr-3">
                {formatDateTime(item.acquisitionDate)}
              </td>
              <td className="py-3 pr-3">
          ...
          <>
            ...
            <div className="space-y-5 px-4 pb-6">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <Detail
                  label="Acquired"
                  value={formatDateTime(item.acquisitionDate)}
                />
                <Detail
                  label="Modified"
                  value={formatDateTime(item.lastModifiedDate)}
                />
              </div>
              <div>
