...
import { Bell, Database, Eye, Loader2, RefreshCw, ShieldCheck } from "lucide-react";
...
function SettingsRoute() {
  ...
  const updateMutation = useMutation({
    mutationFn: (patch: Partial<AppSettings>) => messagingClient.updateSettings(patch),
    onSuccess: async () => {
  ...
  const clearCacheMutation = useMutation({
    mutationFn: () =>
      messagingClient.sendMessage<{ success: true }>({ action: "clearOfferCache" }),
    onSuccess: async () => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 overflow-y-auto px-2 pb-10 pt-2">
      <section>
        <p className="text-sm text-muted-foreground">Account, overlay, alerts, cache</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Epic authentication and library sync state.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            />
            <SettingLine label="Owned items" value={String(health?.ownedItemCount ?? 0)} />
            <SettingLine label="Last sync" value={formatDateTime(health?.lastSyncAt)} />
            <SettingLine label="Sync status" value={health?.lastSyncStatus ?? "idle"} />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Controls injected UI on store.epicgames.com.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <ToggleLine
              label="Owned and duplicate indicators"
              value={Boolean(settings?.overlayEnabled)}
              onToggle={(value) => updateMutation.mutate({ overlayEnabled: value })}
            />
            <Separator />
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  variant="outline"
                  onClick={() => updateMutation.mutate({ country: country || "US" })}
                >
                  Save
                </Button>
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              OS notifications are opt-in. Dashboard reminders still appear in-app.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <ToggleLine
            ...
            <ToggleLine
              ...
              onToggle={(value) => updateMutation.mutate({ dealAlertsEnabled: value })}
            />
            <ToggleLine
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Local browser storage used by this extension.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <SettingLine label="Watchlist items" value={String(health?.watchlistCount ?? 0)} />
            <SettingLine
